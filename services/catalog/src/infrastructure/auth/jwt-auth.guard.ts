...
export class JwtAuthGuard implements CanActivate {
  private readonly issuer = process.env.KEYCLOAK_ISSUER ?? 'http://localhost:8080/realms/mini-food';
  private readonly clientId = process.env.KEYCLOAK_CLIENT_ID ?? 'mini-food-client';
  private readonly jwksUri =
    process.env.KEYCLOAK_JWKS_URI ??
    'http://keycloak:8080/realms/mini-food/protocol/openid-connect/certs';
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!request.user.userId) {
      throw new UnauthorizedException('Token does not contain a user identifier.');
    }
